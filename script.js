...
const translations = {
    en: {
        ...
        resetConfirm: "Are you sure you want to reset all tasks and scores? This cannot be undone.",
        days: ['M', 'T', 'W', 'T', 'F', 'S', 'S'],
    ...
    ar: {
        ...
        resetConfirm: "هل أنت تأكد من إعادة ضبط جميع المهام والنتائج؟ لا يمكن التراجع عن هذا الفعل.",
...
window.openDayDetail = function(childId, dayIdx) {
    ...
    // Switch active day view & reset task states if day changes
    if (child.activeDayIdx !== dayIdx) {
        // Save current tasks state to previous day history
        child.dayHistory[child.activeDayIdx] = JSON.parse(JSON.stringify(child.tasks));
        
        child.activeDayIdx = dayIdx;
        
        // Clear task checkmarks to gray for the selected day
